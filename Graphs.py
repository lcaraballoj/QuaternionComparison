...
def graph_all_euler_quaternions(time):
    ...
    graph.set_ylabel("Compute Time (1e-5 sec)", fontdict={'fontsize':18})
...
def graph_avg_euler_quaternions(eTime, qTime):
    ...
    ax.bar(x_pos, values,
       ...
       ecolor='black',
       capsize=10)
    ax.set_ylabel("Computation Time (1e-5 sec)")       # Labels for y-axis
    ax.set_xticks(x_pos)
...
def graph_all_rodrigues_quaternions(time):
    ...
    graph.set_ylabel("Compute Time (1e-3 sec)", fontdict={'fontsize':18})
    graph.set_xlabel("Iteration", fontdict={'fontsize':18})
    graph.legend(fontsize=18)
...
def graph_avg_rodrigues_quaternions(rTime, qTime):
    ...
    ax.bar(x_pos, values,
       ...
       ecolor='black',
       capsize=10)
    ax.set_ylabel("Computation Time (1e-3 sec)")       # Label for y-axis
    ax.set_xticks(x_pos)
    ax.set_xticklabels(labels)
